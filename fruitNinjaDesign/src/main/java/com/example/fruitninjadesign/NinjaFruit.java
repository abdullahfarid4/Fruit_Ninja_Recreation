...
public class NinjaFruit extends Pane {
    ...
    FileInputStream[] fruit_file = new FileInputStream[3];
    ...
    public void setPathTransition(int millis) {
        for (int i = 0; i < 3; i++) {
            fruit_paths[i] = new PathTransition(Duration.millis(millis), cubicCurve);
    ...
    public void setHalfDrop() {
        for (int i = 0; i < 2; i++) {
            translate_fruit[i] = new TranslateTransition(Duration.millis(3000), fruit_imageview[i + 1]);
